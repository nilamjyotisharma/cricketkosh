...
export async function generateMetadata({ params }) {
  const post = allEditorials.find((post) => post.slug === params.slug);
  if (!post) {
    return;
  }
...
export default function Blog({ params }) {
  const post = allEditorials.find((post) => {
  ...
  const currentIndex = allEditorials.findIndex((p) => p.slug === post.slug);

  // Get the next post
  const nextPost = currentIndex < allEditorials.length - 1 ? allEditorials[currentIndex + 1] : null;
