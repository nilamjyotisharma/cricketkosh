...
export async function generateMetadata({ params }) {
  const post = allReviews.find((post) => post.slug === params.slug);
  if (!post) {
    return;
  }
...
export default function Blog({ params }) {
  const post = allReviews.find((post) => {
  ...
  const currentIndex = allReviews.findIndex((p) => p.slug === post.slug);

  // Get the next post
  const nextPost = currentIndex < allReviews.length - 1 ? allReviews[currentIndex + 1] : null;
